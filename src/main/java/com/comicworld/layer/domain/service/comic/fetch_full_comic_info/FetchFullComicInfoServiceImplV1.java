...
@Service("fetchFullComicInfoServiceImplV1")
public class FetchFullComicInfoServiceImplV1 implements FetchFullComicInfoService {
    ...
    public ResponseEntity<Object> fetch(Long id) {
        Optional<Comic> rs = this.comicService.findByIdWithAllRelationshipsLoadedEagerly(id);

        if(rs.isEmpty())
            return new ResponseEntity<>(ResponseBodyFactoryV1.unprocessableResponseBody(
                    new FailedBodyContentV1(Env.UNPROCESSABLE_EXCEPTION_TYPE, "Comic with ID " + id + " does exist.")
